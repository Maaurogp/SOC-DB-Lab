...
- **Severidad (S):** S1 (Level 4 - Bajo, regla genérica `92052` antes del fix)
- **Criticidad (C):** C3 (Host miembro del dominio `corp.local` — superficie de ataque hacia AD)
- **Confianza (F):** F2 (Corroborado por el log de la operación CALDERA y por la telemetría Sysmon EID1 de cada comando ejecutado)
- **Prioridad Final:** **P3** (S1 + C3 + F2)
- *Justificación:* la severidad nativa es baja porque los comandos de reconocimiento no son intrínsecamente maliciosos, pero al tratarse de un host del dominio no se puede depender de la criticidad del activo para escalar — de ahí la regla local `100012`.
